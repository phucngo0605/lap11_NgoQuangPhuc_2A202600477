    ...
    async def after_model_callback(
        ...
        *,
        ...
        if not filter_result["safe"]:
            ...
            llm_response.content.parts = [types.Part.from_text(filter_result["redacted"])]
            # Update response_text for the next check
            response_text = filter_result["redacted"]

        # 2. If use_llm_judge: call llm_safety_check(response_text)
        if self.use_llm_judge:
            safety_check = await llm_safety_check(response_text)
            if not safety_check["safe"]:
                self.blocked_count += 1
                safe_message = f"The original response was blocked for safety reasons. Reason: {safety_check['verdict']}"
                llm_response.content.parts = [types.Part.from_text(safe_message)]
